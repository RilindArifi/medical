...
const toggleSidebarOne = () => {
  ...
  document.documentElement.classList.toggle('menu-opened', isSidebarOpen.value)
}
  ...
  <div class="header">
    ...
    <ul class="nav user-menu float-end">
      <li class="nav-item dropdown d-none d-md-block">
        <a href="javascript:;" class="dropdown-toggle nav-link" data-bs-toggle="dropdown">
          <span v-if="$i18n.locale.includes('al')" class="fi fi-al fis" title="flag flag-al" data-bs-toggle="tooltip"></span>
          <span v-else class="fi fi-gb fis" title="flag flag-gb" data-bs-toggle="tooltip"></span>
        </a>
        <div class="dropdown-menu notifications">
          <div class="drop-lang">
            <ul class="notification-list d-flex align-items-center">
              <li class="notification-message">
                <a href="javascript:;" @click="onChangeLang('en'); $i18n.locale = 'en'" v-if="$i18n.locale.includes('al')"><span class="fi fi-gb fis" title="flag flag-gb" data-bs-toggle="tooltip"></span> &nbsp;English</a>
                <a href="javascript:;" @click="onChangeLang('al'); $i18n.locale = 'al'" v-else><span class="fi fi-al fis" title="flag flag-al" data-bs-toggle="tooltip"></span> &nbsp;Albania</a>
              </li>
            </ul>
          </div>
        </div>
        ...
        <a href="javascript:;" class="dropdown-toggle nav-link user-link" data-bs-toggle="dropdown">
          <div class="user-names">
            ...
            <span v-for="role in authStore.user.role">
              {{role}} <template v-if="authStore.user.role.length > 1"> </template>
            </span>
          </div>
          <span class="user-img">
    ...
    <div class="dropdown mobile-user-menu float-end">
      ...
      <div class="dropdown-menu dropdown-menu-end">
        ...
        <router-link class="dropdown-item" to="/edit-profile">Edit Profile</router-link>
        <router-link class="dropdown-item" to="/settings/settings">Settings</router-link>
        <button class="dropdown-item" @click.prevent="authStore.logout()">Logout</button>
      </div>
    </div>
  </div>
