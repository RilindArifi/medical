...
export default function useForm(options = {}) {
    ...
    const
        ...
        state = ref(options.state ?? options.initialState),
        rules = ref(options.rules ?? {}),
        v$ = useVuelidate(rules, state, {$lazy: true, $externalResults: errors, $scope: CollectFlag.COLLECT_NONE /* fix when use multiple forms in same component */}),
