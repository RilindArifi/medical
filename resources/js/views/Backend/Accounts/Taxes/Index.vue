...
const
    model = useModelOperations('taxes'),
    isLoading = ref(false),
    data = ref([]),
    state = reactive({
      per_page: 10,
    ...
    deleteAction = (id) => {
      model.deleteAction(id)
          .then(() => { message.success({ content: 'Tax was successfully deleted.' }); fetchData(false) })
    },
    restoreAction = (id) => {
      model.restoreAction(id)
          .then(() => { message.success({ content: 'Tax was successfully restored.' }); fetchData(false) })
    },
    forceDeleteAction = (id) => {
      model.forceDeleteAction(id)
          .then(() => { message.success({ content: 'Tax was successfully force deleted.' }); fetchData(false) })
    },
    enableAction = (id) => {
      model.enableAction(id)
          .then(() => { message.success({ content: `Tax was successfully ${ACTIVE}.` }); fetchData(false) })
    },
    disableAction = (id) => {
      model.disableAction(id)
          .then(() => { message.success({ content: `Tax was successfully ${INACTIVE}.` }); fetchData(false) })
    }
                  ...
                  <div class="col">
                    <div class="doctor-table-blk">
                      ...
                      <div class="doctor-search-blk">
                        ...
                        <div class="add-group">
                          <ButtonGroup model="taxes" :deleted="state.deleted" @on-deleted="stateDeleted" @on-fetch="fetchData(true)"/>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <template #bodyCell="{ column, record }">
                        ...
                        <div class="text-end d-flex justify-content-end align-items-center">
                          <ul class="icons-list mt-3">
                            <template v-if="!state.deleted">
                              <active-model v-if="$can(`enabled taxes`)" @on-enabled="enableAction(record.id)" />
                              <inactive-model v-if="$can(`disabled taxes`)" @on-disabled="disableAction(record.id)"/>
                            </template>
                          </ul>
                        </div>
                      </template>
                      ...
                      <template v-else-if="column.key === 'action'">
                        <div class="text-end d-flex justify-content-end align-items-center">
                          <ul class="icons-list mt-3">
                            <template v-if="!state.deleted">
                              <edit-model :id="record.id" model="taxes" />
                              <delete-model v-if="$can(`delete taxes`)" @on-delete="deleteAction(record.id)"/>
                            </template>
                            <template v-if="state.deleted">
                              <restore-model v-if="$can(`restore taxes`)" @on-restore="restoreAction(record.id)"/>
                              <force-delete-model v-if="$can(`force-delete taxes`)" @on-delete="forceDeleteAction(record.id)"/>
                            </template>
                          </ul>
                        </div>
                      </template>
