...
const
    ...
    onSubmit = async () => {
      ...
      await prepareRequest(() => (modelId ? models.updateAction(modelId, state.value) : models.storeAction(state.value))
          .then((response) => {
            message.success(`Patient was successfully ${modelId ? 'updated' : 'created'}.`)
            router.push({name: 'patients.edit', params:{id: response.data.data.id}})
          }))
    }
...
<template>
  <div class="page-wrapper">
    <div class="content">
      <breadcrumb title="Patients" :text="modelId ? 'Update Patient' : 'Create Patient'" path="/patients" />
      <div class="row">
        <div class="col-sm-12">
          <div class="card">
            <div class="card-body">
                  ...
                  <div class="row">
                    <div class="col-12">
                      <div class="form-heading">
                        <h4>{{modelId ? 'Update Patient' : 'Add Patient'}}</h4>
                      </div>
                    </div>
                    ...
                    <div class="col-12">
                      <div class="doctor-submit text-end">
                        ...
                        <router-link :to="{ name: 'patients.index' }" class="btn btn-primary cancel-form"> Cancel </router-link>
                      </div>
                    </div>
                  </div>
                </form>
