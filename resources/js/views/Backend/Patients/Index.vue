...
const
    model = useModelOperations('patients'),
    isLoading = ref(false),
    data = ref([]),
    state = reactive({
      per_page: 10,
    ...
    deleteAction = (id) => {
      model.deleteAction(id)
          .then(() => { message.success({ content: 'Patient was successfully deleted.' }); fetchData(false) })
    },
    restoreAction = (id) => {
      model.restoreAction(id)
          .then(() => { message.success({ content: 'Patient was successfully restored.' }); fetchData(false) })
    },
    forceDeleteAction = (id) => {
      model.forceDeleteAction(id)
          .then(() => { message.success({ content: 'Patient was successfully force deleted.' }); fetchData(false) })
    },
    enableAction = (id) => {
      model.enableAction(id)
          .then(() => { message.success({ content: `Patient was successfully ${ACTIVE}.` }); fetchData(false) })
    },
    disableAction = (id) => {
      model.disableAction(id)
          .then(() => { message.success({ content: `Patient was successfully ${INACTIVE}.` }); fetchData(false) })
    }
...
<template>
  <div class="page-wrapper">
    <div class="content">
      <breadcrumb title="Create Patient" text="Patients" path="/patients/create" />
                  ...
                  <div class="col">
                    <div class="doctor-table-blk">
                      ...
                      <div class="doctor-search-blk">
                        ...
                        <div class="add-group">
                          <ButtonGroup model="patients" :deleted="state.deleted" @on-deleted="stateDeleted" @on-fetch="fetchData(true)"/>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <template #bodyCell="{ column, record }">
                      <template v-if="column.key === 'Name'">
                        <div class="profile-image">
                          <router-link :to="{name: 'patients.edit', params:{id: record.id}}">
                        ...
                        <div class="text-end d-flex justify-content-end align-items-center">
                          <ul class="icons-list mt-3">
                            <template v-if="!state.deleted">
                              <active-model v-if="$can(`enabled patients`)" @on-enabled="enableAction(record.id)" />
                              <inactive-model v-if="$can(`disabled patients`)" @on-disabled="disableAction(record.id)"/>
                            </template>
                          </ul>
                        </div>
                      </template>
                      ...
                      <template v-else-if="column.key === 'action'">
                        <div class="text-end d-flex justify-content-end align-items-center">
                          <ul class="icons-list mt-3">
                            <template v-if="!state.deleted">
                              <edit-model :id="record.id" model="patients" />
                              <delete-model v-if="$can(`delete patients`)" @on-delete="deleteAction(record.id)"/>
                            </template>
                            <template v-if="state.deleted">
                              <restore-model v-if="$can(`restore patients`)" @on-restore="restoreAction(record.id)"/>
                              <force-delete-model v-if="$can(`force-delete patients`)" @on-delete="forceDeleteAction(record.id)"/>
                            </template>
                          </ul>
                        </div>
                      </template>
