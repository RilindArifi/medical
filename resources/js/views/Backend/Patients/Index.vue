...
const model = useModelOperations("patients"),
    isLoading = ref(false),
    data = ref([]),
    state = reactive({
        per_page: 10,
    ...
    deleteAction = (id) => {
        model.deleteAction(id).then(() => {
            message.success({ content: "Patient was successfully deleted." });
            fetchData(false);
        });
    },
    restoreAction = (id) => {
        model.restoreAction(id).then(() => {
            message.success({ content: "Patient was successfully restored." });
            fetchData(false);
        });
    },
    forceDeleteAction = (id) => {
        model.forceDeleteAction(id).then(() => {
            message.success({
                content: "Patient was successfully force deleted.",
            });
            fetchData(false);
        });
    },
    enableAction = (id) => {
        model.enableAction(id).then(() => {
            message.success({ content: `Patient was successfully ${ACTIVE}.` });
            fetchData(false);
        });
    },
    disableAction = (id) => {
        model.disableAction(id).then(() => {
            message.success({
                content: `Patient was successfully ${INACTIVE}.`,
            });
            fetchData(false);
        });
    };
