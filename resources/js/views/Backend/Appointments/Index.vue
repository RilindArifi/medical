...
const
    model = useModelOperations('appointments'),
    isLoading = ref(false),
    data = ref([]),
    state = reactive({
      per_page: 10,
    ...
    deleteAction = (id) => {
      model.deleteAction(id)
          .then(() => { message.success({ content: 'Appointment was successfully deleted.' }); fetchData(false) })
    },
    restoreAction = (id) => {
      model.restoreAction(id)
          .then(() => { message.success({ content: 'Appointment was successfully restored.' }); fetchData(false) })
    },
    forceDeleteAction = (id) => {
      model.forceDeleteAction(id)
          .then(() => { message.success({ content: 'Appointment was successfully force deleted.' }); fetchData(false) })
    },
    enableAction = (id) => {
      model.enableAction(id)
          .then(() => { message.success({ content: 'Appointment was successfully approved.' }); fetchData(false) })
    },
    disableAction = (id) => {
      model.disableAction(id)
          .then(() => { message.success({ content: 'Appointment was successfully rejected.' }); fetchData(false) })
    },
...
<template>
  <div class="page-wrapper">
    <div class="content">
      <!-- Page Header -->
      <breadcrumb :title="$t('appointments.newAppointment')" :text="$t('appointments.apps')" path="/appointments/create" />
      <!-- /Page Header -->
                ...
                <div class="row align-items-center">
                  <div class="col">
                    <div class="doctor-table-blk">
                      <h3>{{$t('appointments.appointmentList')}}</h3>
                      <div class="doctor-search-blk">
                        <div class="top-nav-search table-search-blk">
                          <form>
                            <input v-model="state.search" type="text" class="form-control" :placeholder="$t('table.search')" />
                            <a class="btn"
                        ...
                        <div class="add-group">
                          <ButtonGroup model="appointments" :deleted="state.deleted" @on-deleted="stateDeleted" @on-fetch="fetchData(true)"/>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="row">
                    <div class="col-12 col-md-6 col-xl-4">
                      <div class="input-block local-forms">
                        ...
                        <patient-select
                            v-model:value="state.patient_id"
                        />
                      </div>
                    </div>
                    <div class="col-12 col-md-6 col-xl-4">
                  ...
                  <template #bodyCell="{ column, record }">
                      <template v-if="column.key === 'Name'">
                        <div class="profile-image">
                          <router-link :to="{name: 'appointments.edit', params:{id: record.id}}">
                            <vue-avatar v-if="record.patient" :user="record.patient" />
                            {{ record.patient?.name }}
                          </router-link>
                        </div>
                      </template>
                      <template v-else-if="column.key === 'Consulting Doctor'">
                        <div class="profile-image">
                          <router-link to="/profile">
                            <vue-avatar v-if="record.doctor" :user="record.doctor" />
                            {{ record.doctor?.name }}
                          </router-link>
                        </div>
                      </template>
                      <template v-else-if="column.key === 'Mobile'">
                        <p class="mt-3">{{ record.patient?.mobile }}</p>
                      </template>
                      ...
                      <template v-else-if="column.key === 'quick-access' && !state.deleted">
                        <div class="text-end d-flex justify-content-end align-items-center">
                          <ul class="icons-list mt-3">
                            <template v-if="!state.deleted">
                              <approve-model v-if="$can(`enabled appointments`)" @on-enabled="enableAction(record.id)" titleTooltip="Approve"/>
                              <reject-model v-if="$can(`disabled appointments`)" @on-disabled="disableAction(record.id)" titleTooltip="Reject"/>
                            </template>
                          </ul>
                        </div>
                      </template>
                      <template v-else-if="column.key === 'action'">
                        <div class="text-end d-flex justify-content-end align-items-center">
                          <ul class="icons-list mt-3">
                            <template v-if="!state.deleted">
                              <edit-row :id="record.id" model="appointments" />
                              <delete-row v-if="$can(`delete appointments`)" @on-delete="deleteAction(record.id)"/>
                            </template>
                            <template v-if="state.deleted">
                              <restore-row v-if="$can(`restore appointments`)" @on-restore="restoreAction(record.id)"/>
                              <force-delete-row v-if="$can(`force-delete appointments`)" @on-delete="forceDeleteAction(record.id)"/>
                            </template>
                          </ul>
                        </div>
                      </template>
