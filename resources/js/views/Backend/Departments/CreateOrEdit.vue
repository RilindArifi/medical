...
const
    ...
    onSubmit = async () => {
      ...
      await prepareRequest(() => (modelId ? models.updateAction(modelId, state.value) : models.storeAction(state.value))
          .then((response) => {
            message.success(`Department was successfully ${modelId ? 'updated' : 'created'}.`)
            return router.push({name: 'departments.edit', params:{id: response.data.data.id}})
          }))
    }
...
<template>
  <div class="page-wrapper">
    <div class="content">
      <!-- Page Header -->
      <breadcrumb title="Departments" :text="modelId ? 'Update Department' : 'Add Department'" :path="{name: 'departments.index'}" />
      <!-- /Page Header -->
      <div class="row">
        <div class="col-sm-12">
          <div class="card">
                  ...
                  <div class="row">
                    <div class="col-12">
                      <div class="form-heading">
                        <h4>{{modelId ? 'Update Department' : 'Add Department'}}</h4>
                      </div>
                    </div>
                    ...
                    <div class="col-12 col-sm-12">
                      <div class="input-block local-forms">
                        ...
                        <textarea v-model="state.description" class="form-control" rows="3" cols="30" :class="{ 'is-invalid': fieldError('description') }"></textarea>
                        <div class="invalid-feedback">{{ fieldError('description') }}</div>
                      </div>
                    </div>
                    <div class="col-12 col-md-6 col-xl-6">
                    ...
                    <div class="col-12">
                      <div class="doctor-submit text-end">
                        ...
                        <router-link :to="{name: 'departments.index'}" type="submit" class="btn btn-primary cancel-form">Cancel</router-link>
                      </div>
                    </div>
                  </div>
                </form>
