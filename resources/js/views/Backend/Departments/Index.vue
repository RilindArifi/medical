...
const
    model = useModelOperations('departments'),
    isLoading = ref(false),
    data = ref([]),
    state = reactive({
      per_page: 10,
    ...
    deleteAction = (id) => {
      model.deleteAction(id)
          .then(() => { message.success({ content: 'Department was successfully deleted.' }); fetchData(false) })
    },
    restoreAction = (id) => {
        model.restoreAction(id)
          .then(() => { message.success({ content: 'Department was successfully restored.' }); fetchData(false) })
    },
    forceDeleteAction = (id) => {
      model.forceDeleteAction(id)
          .then(() => { message.success({ content: 'Department was successfully force deleted.' }); fetchData(false) })
    },
    enableAction = (id) => {
      model.enableAction(id)
          .then(() => { message.success({ content: `Department was successfully ${ACTIVE}.` }); fetchData(false) })
    },
    disableAction = (id) => {
      model.disableAction(id)
          .then(() => { message.success({ content: `Department was successfully ${INACTIVE}.` }); fetchData(false) })
    }
...
<template>
  <div class="page-wrapper">
    <div class="content">
      <!-- Page Header -->
      <breadcrumb title="Departments" text="Department List" path="department-list" />
      <!-- /Page Header -->
                  ...
                  <div class="col">
                    <div class="doctor-table-blk">
                      ...
                      <div class="doctor-search-blk">
                        ...
                        <div class="add-group">
                          <ButtonGroup model="departments" :deleted="state.deleted" @on-deleted="stateDeleted" @on-fetch="fetchData(true)"/>
                        </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <template #bodyCell="{ column, record }">
                        ...
                        <div class="text-end d-flex justify-content-end align-items-center">
                          <ul class="icons-list mt-3">
                            <template v-if="!state.deleted">
                              <active-model v-if="$can(`enabled departments`)" @on-enabled="enableAction(record.id)" />
                              <inactive-model v-if="$can(`disabled departments`)" @on-disabled="disableAction(record.id)"/>
                            </template>
                          </ul>
                        </div>
                      </template>
                      ...
                      <template v-else-if="column.key === 'action'">
                        <div class="text-end d-flex justify-content-end align-items-center">
                          <ul class="icons-list mt-3">
                            <template v-if="!state.deleted">
                              <edit-model :id="record.id" model="departments" />
                              <delete-model v-if="$can(`delete departments`)" @on-delete="deleteAction(record.id)"/>
                            </template>
                            <template v-if="state.deleted">
                              <restore-model v-if="$can(`restore departments`)" @on-restore="restoreAction(record.id)"/>
                              <force-delete-model v-if="$can(`force-delete departments`)" @on-delete="forceDeleteAction(record.id)"/>
                            </template>
                          </ul>
                        </div>
                      </template>
