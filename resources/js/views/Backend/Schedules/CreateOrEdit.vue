...
const
    ...
    onSubmit = async () => {
      ...
      await prepareRequest(() => (modelId ? models.updateAction(modelId, state.value) : models.storeAction(state.value))
          .then((response) => {
            message.success(`Schedule was successfully ${modelId ? 'updated' : 'created'}.`)
            router.push({name: 'schedules.edit', params:{id: response.data.data.id}})
          }))
    }
...
<template>
  <div class="page-wrapper">
    <div class="content">
      <!-- Page Header -->
      <breadcrumb title="Schedules" text="New Schedule" path="/schedules" />
      <!-- /Page Header -->
      <div class="row">
        <div class="col-sm-12">
          <div class="card">
                ...
                <div class="row">
                  ...
                  <div class="col-12 col-md-6 col-xl-6">
                    <div class="input-block local-forms">
                      <label>Available Days <span class="login-danger">*</span></label>
                        <schedule-select
                            v-model:value="state.available_days"
                            :class="{ 'is-invalid': fieldError('available_days') }"
                        />
                      <div class="invalid-feedback">{{ fieldError('available_days') }}</div>
                    </div>
                  </div>
                  <div class="col-12 col-md-6 col-xl-6">
                    <div class="input-block local-forms">
                  ...
                  <div class="col-12">
                    <div class="doctor-submit text-end">
                      ...
                      <router-link :to="{name: 'schedules.index'}" type="submit" class="btn btn-primary cancel-form">Cancel</router-link>
                    </div>
                  </div>
                </div>
                </div>
