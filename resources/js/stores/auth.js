...
export const useAuthStore = defineStore('auth', {
    ...
    actions: {
        ...
        loadData(data) {
            ...
            Cookies.set('token', data.token, { expires: new Date(new Date().setDate(new Date().getDate() + (data.remember ? 365 : 1 )))});
        },
        async logout() {
            try {
        ...
        async setRules() {
            const permissions = this.user.permissions?.map(permission => permission.name) ?? [];
