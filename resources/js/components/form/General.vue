...
const
    ...
    required = !route.path.includes('patients') && !route.path.includes('appointments'),
...
<template>
  ...
  <div class="col-12 col-md-6 col-xl-6">
    <div class="input-block local-forms">
      ...
      />
      <div class="invalid-feedback">{{ getError('password_confirmation') }}</div>
    </div>
  </div>
  <div class="col-12 col-md-6 col-xl-6">
    <div class="input-block local-forms cal-icon">
      <label>Date Of Birth <span v-if="required" class="login-danger">*</span></label>
      <a-date-picker
  ...
  <div class="col-12 col-md-6 col-xl-6">
    <div class="input-block select-gender">
      <label class="gen-label">Gender<span v-if="required" class="login-danger"> *</span></label>
      <radio-gender
  ...
  <template v-if="required">
    <div class="col-12 col-md-6 col-xl-6">
      <div class="input-block local-forms">
        ...
        />
        <div class="invalid-feedback">{{ fieldError('education') }}</div>
      </div>
    </div>
    <div class="col-12 col-md-6 col-xl-6">
      <div class="input-block local-forms">
        <label>Department <span v-if="required && staffs" class="login-danger">*</span></label>
        <department-select
            v-model:value="state.department_id"
            :class="{ 'is-invalid': fieldError('department_id') }"
        />
        <div class="invalid-feedback">{{ fieldError('department_id') }}</div>
      </div>
    </div>
  </template>
</template>
